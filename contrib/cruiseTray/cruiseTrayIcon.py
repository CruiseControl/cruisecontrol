...
# The value of the 'url' option must be start with
...
"""            
    ...
    def OnInfo(self,evt):
        dlg1 = wx.MessageDialog(self, 'CruiseControl Tray Icon made by CC-CONTRIB, modified by Ali Eyertas (eyertas@gmx.de) 2004','Info', wx.OK | wx.ICON_INFORMATION)
    ...
    def OnConfOk(self,evt):
        ...
        if self.teUrl.GetValue().__len__()== 0:
            dlg2 = wx.MessageDialog(self.f, "Sie haben keine URL eingetragen!", 'Cruise Control Message', wx.OK | wx.ICON_ERROR | wx.STAY_ON_TOP)
            dlg2.ShowModal()
            dlg2.Destroy()
        ...
        if self.teUrl.GetValue().find("http://") != 0: 
            dlg2 = wx.MessageDialog(self.f, "Die URL muss mit http:// anfangen.", 'Cruise Control Message', wx.OK | wx.ICON_ERROR | wx.STAY_ON_TOP)
            dlg2.ShowModal()
            dlg2.Destroy()
            self.teUrl.SetValue(self.configurationOptions.cruiseUrl)
            return
        if self.teUrl.GetValue().__len__()<= 7:
            dlg2 = wx.MessageDialog(self.f, "Die URL ist unvollständig!", 'Cruise Control Message', wx.OK | wx.ICON_ERROR | wx.STAY_ON_TOP)
            dlg2.ShowModal()
            dlg2.Destroy()
            self.teUrl.SetValue(self.configurationOptions.cruiseUrl)
            return
        ...
        if self.teFaStr.GetValue().__len__()== 0:
            dlg2 = wx.MessageDialog(self.f, "Sie haben keinen FAILSTRING eingetragen!", 'Cruise Control Message', wx.OK | wx.ICON_ERROR | wx.STAY_ON_TOP)
            dlg2.ShowModal()
            dlg2.Destroy()
        ...
        if self.teRef.GetValue()*1000 < 20000:
            dlg2 = wx.MessageDialog(self.f, "Refresh Time ist zu kurz! Mindestens 20 Sekunden!", 'Cruise Control Message', wx.OK | wx.ICON_ERROR | wx.STAY_ON_TOP)
            dlg2.ShowModal()
            dlg2.Destroy()
    ...
    def CheckBuild(self, evt):
    	...
    	if self.buildIsBrokenStatus:
            ...
            self.tbicon.SetIcon(icon, 'Build ist FEHLERHAFT')
        else:
            icon = wx.Icon('green.ico', wx.BITMAP_TYPE_ICO)
            self.tbicon.SetIcon(icon, 'Build ist OK')
    ...
    def stillBroken(self):
        #self.showMessage("Build ist immernoch Fehlerhaft")
        pass

    def buildFixed(self):
        self.showMessage("Fehlerhafter Build wurde verbessert")

    def buildBroken(self):
        self.showMessage("Build wurde soeben Fehlerhaft erstellt")

    def stillWorking(self):
        #self.showMessage("Build wird momentan erstellt")
        pass
